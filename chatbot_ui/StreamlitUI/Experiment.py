...
MONGODB_ATLAS_CLUSTER_URI = 'mongodb+srv://gaurav:123@legal.uetqqox.mongodb.net/'
...
ATLAS_VECTOR_SEARCH_INDEX_NAME = "vector_index_phub"
...
url_retrieval = "http://54.144.64.60:8001/get_response"
url_vectorization_classify = "http://54.144.64.60:8000/classify"
url_vectorization_upload = "http://54.144.64.60:8000/upload"
...
def classify_text(response_json):
    contract_type = response_json['contract_type']
    summary = response_json['summary']
    return contract_type,summary,response_json
...
if __name__ == "__main__":
    ...
    if pdf_file is not None and not st.session_state["pdf_processed"]:
        ...
        response_url = requests.post(url_vectorization_upload, files={'file': open(pdf_file.name, 'rb')})

        response_classify = requests.post(url_vectorization_classify, files={'file': open(pdf_file.name, 'rb')})
        #st.markdown(f'<small>{response_classify.json()}</small>', unsafe_allow_html=True)
        # Remove the file from the local system
        os.remove(pdf_file.name)
        #print(response_classify)
        try:
            classification_result,summary_result,extraction_result = classify_text(response_classify.json())
        except Exception:
            classification_result,summary_result,extraction_result = "Invalid contract", "Invalid contract", "Invalid contract"
        ...
        try:
            ...
            st.markdown('<small> **Invalid contract** </small>', unsafe_allow_html=True)
    ...
    if user_input := st.chat_input("What is your question?"):
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("PeritusHub's bot is thinking ..."):
                ...
                #st.markdown("assistant_response",assistant_response['response'])
                st.session_state["chat_history"]=chat_history[-20:]
            # Simulate stream of response with milliseconds delay
            for chunk in assistant_response.json()['response'].split():
                full_response += chunk + " "
                time.sleep(0.05)
